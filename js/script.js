...
async function getSongs(folder) {
    ...
    for (let index = 0; index < as.length; index++) {
        ...
        if (element.href.endsWith(".mp3")) {
            let dirtyFilename = element.href.split("/").pop();
            let cleanFilename = decodeURIComponent(dirtyFilename).split("\\").pop();
            songs.push(encodeURIComponent(cleanFilename));
        }
    }
...
async function main() {
    ...
    if (songs.length > 0) {
        ...
        console.error("No songs found in the default directory 'songs/ncs'.");
    }
    ...
    previous.addEventListener("click", () => {
        currentSong.pause();
        let currentTrack = encodeURIComponent(decodeURIComponent(currentSong.src.split("/").pop()));
